...
/**
 * This application window class, allows customers to view products that they have added to their wish list.
 * Customers can buy products from here, and can also refine products according to a search criteria.
 * Customers can also remove the product from their wish list.
 * @author Sachin
 *
 */
...
public class WishListScreen {
	...
	private JList<String> wishList; // JList which contains String references to the products currently in the customer's wish list.
	String primaryKey; //The email of customer that is currently logged in.
	...
	private DefaultListModel<String> wishListModel; //DefaultListModel to implement the JList.
	...
	private void initialize() {
		...
		frame.getContentPane().add(scrollPane);
				...
				DefaultListCellRenderer renderer = (DefaultListCellRenderer) wishList.getCellRenderer();
				renderer.setHorizontalAlignment(SwingConstants.CENTER);
				
				/**
				 * wishList has a selection listener, to determine which product
				...
				wishList.addListSelectionListener(new ListSelectionListener() {
					...
					public void valueChanged(ListSelectionEvent event) {
						...
						if (wishList.getSelectedValue() != null) {
							productID = Integer.parseInt(wishList.getSelectedValue().replaceAll("\\D+", ""));
						}
						...
						try {
							 ...
							 * Construct the query. Query gets all the products that the customer has in their wish list
							...
							String query = "SELECT Products.*, Sells.Email FROM Products INNER JOIN Wish_List ON Products.ID = Wish_List.ID "
									+ "INNER JOIN Sells ON Sells.ID = Products.ID WHERE Products.ID = ?";
							PreparedStatement pst = connection.prepareStatement(query);
							pst.setInt(1, productID);
							ResultSet rs = pst.executeQuery();
							
							/**
							 * While there are results, set the product's name, description, price, quantity and seller email to the appropriate labels.
							 */
		...
		wishList.setSelectedIndex(0);
		wishList.setForeground(new Color(204, 204, 204));
		wishList.setFont(new Font("Century Gothic", Font.BOLD, 14));
		wishList.setBackground(new Color(36, 47, 65));

		JLabel lblWishList = new JLabel("Wish List", SwingConstants.CENTER);
		lblWishList.setForeground(new Color(204, 204, 204));
		lblWishList.setFont(new Font("Century Gothic", Font.BOLD, 20));
		lblWishList.setBounds(10, 56, 288, 26);
		...
		panel_2.addMouseListener(new MouseAdapter() {
			...
			/**
			 * When this panel is clicked, the customer removeProductFromWishList method is called.
			 * This removes the product from the customer's wish list.
			 * The setDetails method is used to refresh the JList.
			 */
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				try {
					...
					if(Customer.getInstance().removeProductFromWishList(productID, primaryKey)) {
						...
						JOptionPane.showMessageDialog(null, "Product removed from your wish list!");
		...
		String[] refineTypes = {"ID", "Price: Low to High", "Price: High to Low", "Quantity: Least to Most", "Quantity: Most to Least", "Alphabetically"};
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				try {
					...
					if(refineComboBox.getSelectedItem() == "ID") {
						...
						/**
						 * Make a connection to database.
						 * Construct the query. Query used to sort products in the list by ID in ascending order
						 * Use a prepared statement to setup query.
						 * set the primary key in the query
						 * Execute the update query.
						 */
						connection = dbConnection.dbConnector();
						String query = "SELECT Products.ID, Products.Name FROM Products INNER JOIN Wish_List ON Products.ID = Wish_List.ID WHERE Email = ? ORDER BY Products.ID";
						PreparedStatement pst = connection.prepareStatement(query);
						pst.setString(1, primaryKey);
						ResultSet rs = pst.executeQuery();

						wishListModel.removeAllElements(); //Remove any elements, as the list will be recreated.
						...
						while (rs.next()) {
							wishListModel.addElement("(id:" + rs.getInt("ID") + ") " + rs.getString("Name"));
						}
						
						wishList.setSelectedIndex(0); //Set the selected index to 0.
						...
						connection = dbConnection.dbConnector();
						String query = "SELECT Products.ID, Products.Name FROM Products INNER JOIN Wish_List ON Products.ID = Wish_List.ID WHERE Email = ? ORDER BY Price";
						PreparedStatement pst = connection.prepareStatement(query);
						pst.setString(1, primaryKey);
						ResultSet rs = pst.executeQuery();
						...
						connection = dbConnection.dbConnector();
						String query = "SELECT Products.ID, Products.Name FROM Products INNER JOIN Wish_List ON Products.ID = Wish_List.ID WHERE Email = ? ORDER BY Price DESC";
						PreparedStatement pst = connection.prepareStatement(query);
						pst.setString(1, primaryKey);
						ResultSet rs = pst.executeQuery();
						...
						connection = dbConnection.dbConnector();
						String query = "SELECT Products.ID, Products.Name FROM Products INNER JOIN Wish_List ON Products.ID = Wish_List.ID WHERE Email = ? ORDER BY Quantity";
						PreparedStatement pst = connection.prepareStatement(query);
						pst.setString(1, primaryKey);
						ResultSet rs = pst.executeQuery();
						...
						connection = dbConnection.dbConnector();
						String query = "SELECT Products.ID, Products.Name FROM Products INNER JOIN Wish_List ON Products.ID = Wish_List.ID WHERE Email = ? ORDER BY Quantity DESC";
						PreparedStatement pst = connection.prepareStatement(query);
						pst.setString(1, primaryKey);
						ResultSet rs = pst.executeQuery();
						...
						connection = dbConnection.dbConnector();
						String query = "SELECT Products.ID, Products.Name FROM Products INNER JOIN Wish_List ON Products.ID = Wish_List.ID WHERE Email = ? ORDER BY Name";
						PreparedStatement pst = connection.prepareStatement(query);
						pst.setString(1, primaryKey);
						ResultSet rs = pst.executeQuery();
	...
	/**
	 * This method is used to setup the wishList.
	...
	private void setDetails() {
		try {
			...
			wishListModel = new DefaultListModel<String>(); //Initialise the DLM for the productsList.
			
			/**
			 * Make a connection to database.
			 * Construct the query. Query gets all the products from the customer's wish list.
			...
			connection = dbConnection.dbConnector();
			String query = "SELECT Products.ID, Products.Name FROM Products INNER JOIN Wish_List ON Products.ID = Wish_List.ID WHERE Email = ?";
			PreparedStatement pst = connection.prepareStatement(query);
			pst.setString(1, primaryKey);
			ResultSet rs = pst.executeQuery();
			
			wishListModel.removeAllElements(); //Remove any elements if there are any.
			
			/**
			 * While there are results, create a string reference to the products ID and name, and add it to the DLM
			 * Set the model for the wishList to the DLM.
			 * Set the selected index to the first item in the list.
			 */
